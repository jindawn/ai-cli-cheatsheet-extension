...
修改任何工具数据、官方清单、案例文案或新增/更新流程前，必须先阅读并严格遵守 [`OFFICIAL_DATA_POLICY.md`](OFFICIAL_DATA_POLICY.md)。该政策要求确定性官方入口零遗漏、逐例第一方 grounding、内容哈希审校、预览确认和原子应用，不能由模型输出、缓存或版本信号绕过。
...
- **数据写入不可降级。** 新增/更新只有在官方清单闭合、覆盖完整、场景审校通过并由用户确认预览后才能写入。手动改数据也必须同步更新官方清单与审校快照，并通过 `node tools/validate-data.js`。
