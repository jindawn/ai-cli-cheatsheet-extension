...
3. Host 先用受信任的声明式适配器闭合官方入口清单，再把核验资料交给模型结构化整理；签名适配器只使用固定参数数组和受控输出驱动，用户确认的通用调用只能使用桥接自带的固定调用模板，且需先试调用确认该工具能响应其中一种。两者都禁止 Shell 字符串及模型返回内容执行。
...
- 基础查询不依赖 Native Host；新增和更新功能需要商店版图形安装包（已签名或未签名，未签名时需在系统安全提示中确认一次）或源码版开发安装脚本提供本机桥接。浏览器扩展无法自行注册本机程序，这是 Native Messaging 的安全边界，所以这一步必须在浏览器之外完成。
