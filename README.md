...
当前包含 **17 个工具、1000 余条数据**（准确数字以 `node tools/validate-data.js` 的输出为准），每条均提供语义关键词、用法示例和明确核验状态，并支持平台适配、收藏、最近使用和工具筛选。基础查询完全在浏览器本地完成，不上传搜索词或使用记录；只有可选的“新增工具 / 检查更新”功能会调用本机 Claude Code 或配置的 Anthropic 兼容 API。
...
| 管理 | 切换平台、启用工具、查看来源、逐次比对官方入口清单或新增工具 |
...
- “检查官方更新”每次都会重新读取官方入口目录；只有官方入口全部覆盖后才会显示暂无更新。版本号仅决定是否需要复核语义，不能掩盖历史遗漏。
- 官方目录、Native Host、Node.js、Claude Code 或网络环境不可用时，管理页会显示失败阶段、实际原因和修复建议，不会把缓存结果标成最新。
