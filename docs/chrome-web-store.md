...
2. 先发布并验证 `v1.7.8` 的 macOS、Windows、Linux 桥接资产及 SHA-256，再上传 `ai-cli-cheatsheet-store-v1.7.8.zip`。
3. 首次提交和发布在开发者后台人工完成。
...
- 可选使用桥接动态检测的 AI 环境查询并新增工具、检查官方更新；常见 AI CLI 可一键检测，其他工具输入名称后自动检测，找到后经一次风险确认以无参数通用模式尝试调用；已验证适配器仍优先使用，应用前显示差异与风险确认。
