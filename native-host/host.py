...
# Shell-proper structural axes. Shell covers the interpreter and terminal
...
SHELL_BATCHES = [
    ...
    {"id": "zsh-builtins", "label": "zsh 内置命令", "topics": ["builtins"], "scope": "zsh 专有内置：autoload、setopt/unsetopt、zstyle、whence、where、typeset、print、vared、zmodload、functions，以及与 bash 的差异（只收录 zsh 解释器本体）"},
    {"id": "functions-aliases", "label": "函数与别名", "topics": ["scripting", "environment"], "scope": "function 定义、return、local；alias/unalias、别名 vs 函数、zsh autoload 自动加载函数"},
    {"id": "control-flow", "label": "控制流关键字", "topics": ["syntax", "scripting"], "scope": "关键字 if/elif/else/fi、for、while、until、case/esac、select、function；{ } 与 ( ) 分组、&&、||、;"},
    {"id": "variables-parameters", "label": "变量与参数展开", "topics": ["syntax", "scripting"], "scope": "declare/typeset、readonly、export、${VAR:-默认}、${#VAR}、${VAR%pat}、$@/$*/$#/$?/$$/$!、位置参数"},
    {"id": "expansion-redirection", "label": "展开与重定向", "topics": ["syntax"], "scope": "管道 |、重定向 > >> <、heredoc <<、herestring <<<、2>&1、&>、/dev/null、命令替换 $(...)、进程替换 <(...)、算术 $((...))"},
    {"id": "globbing-quoting", "label": "通配与引用", "topics": ["syntax"], "scope": "通配符 *、?、[ ]；花括号展开 {a,b}；波浪号 ~；单引号/双引号与转义；bash extglob、zsh 扩展通配与限定符"},
    {"id": "shortcuts-movement", "label": "行编辑快捷键", "topics": ["shortcuts"], "scope": "命令行行编辑（emacs 键位）：Ctrl+A、Ctrl+E、Ctrl+B、Ctrl+F、Alt+B、Alt+F、Ctrl+U、Ctrl+K、Ctrl+W、Ctrl+D、Ctrl+L、Ctrl+T"},
    {"id": "history", "label": "历史命令与展开", "topics": ["history", "shortcuts"], "scope": "history、fc、HISTSIZE/HISTFILE/HISTCONTROL；历史展开 !!、!$、!^、!n、^old^new；Ctrl+R 反搜、Ctrl+P/Ctrl+N"},
    {"id": "job-control", "label": "作业控制与信号", "topics": ["jobs"], "scope": "作业控制：jobs、fg、bg、wait、disown、suspend、trap 捕获信号、kill 内建发送信号、& 后台运行"},
    {"id": "completion", "label": "补全系统", "topics": ["completion"], "scope": "Tab 补全；bash-completion、complete/compgen；zsh compinit/compdef/zstyle 补全配置"},
    {"id": "config-files", "label": "配置文件", "topics": ["config"], "scope": "~/.bashrc、~/.bash_profile、~/.profile、~/.zshrc、~/.zprofile、~/.zshenv 等：加载顺序、登录式/交互式区别、何时用哪个"},
    {"id": "environment-path", "label": "环境变量与 PATH", "topics": ["environment"], "scope": "export、env、printenv；PATH、HOME、SHELL、LANG、IFS、PS1；修改 PATH、source/. 重新加载"},
...
{{
  ...
  "items": [
    {{
      ...
      "shell": {{
        "layer": "仅 Shell 聚合工具使用：syntax|keyword|builtin|option|shortcut|config",
        "family": "仅 Shell 聚合工具使用：只能是 posix-sh、bash、zsh（Shell 解释器本体）；禁止 coreutils、grep、sed、find、tar 等外部工具族",
        "portability": "仅 Shell 聚合工具使用：posix|bash|zsh|cross-platform",
        "topic": "仅 Shell 聚合工具使用：builtins|syntax|shortcuts|config|environment|history|completion|scripting|jobs|troubleshooting"
      }},
...
3. 覆盖本批次的 Shell 内置命令、关键字、语法、快捷键、配置项或排错点，应覆盖该 scope 的常用全集；不要包含极冷门历史兼容细节。
3a. Shell 内置命令、关键字、快捷键、配置项都是稳定且有据可查的内容，请尽量覆盖常用全集，不要因保守而漏掉高频内置或选项（但仍不得编造不存在的命令或选项）。
3b. 快捷键、操作符/重定向符号、关键字、环境变量这类原子条目（本批次 scope 中以顿号列出的具体项），请为每一项各生成一个独立 item，不要合并成一条——用户通常逐个查它们。例如 Ctrl+A、Ctrl+E 各一条；管道 | 与重定向 > 各一条；if、for、while 各一条；PATH、HOME 各一条。
4. 本批次目标 10-18 条，尽量覆盖该 scope 的常用项；最多输出 {max_items} 个 items。超过上限时优先保留最常查、最易错、最能代表该 scope 的条目。
5. 所有 item 必须有 keywords 和 evidenceRefs。keywords 用中文高频检索词（如 终端、环境变量、配置、快捷键、补全、历史、排错、退出码、管道、重定向）并带上对应 shell 名（bash/zsh/sh/POSIX）。核心、高频、危险、易错、平台差异条目必须有 examples；普通条目可省略 examples。
